...
class Pessoa(Base):
    ...
    email_institucional: Mapped[str] = mapped_column(VARCHAR(50), unique=True,
                                                     nullable=False)

class PessoaFisica(Base):
    __tablename__ = "pessoa_fisica"
    codigo: Mapped[int] = mapped_column(INTEGER, ForeignKey(Pessoa.codigo), 
                                                    primary_key=True)
    ...
    email_pessoal: Mapped[str] = mapped_column(VARCHAR(50), 
...
class Escola(Base):
    ...
    codigo_escola: Mapped[int] = mapped_column(SMALLINT, primary_key=True, 
                                               autoincrement=True)
    nome: Mapped[str] = mapped_column(VARCHAR(256), nullable=False)
    filial_codigo: Mapped[int] = mapped_column(INTEGER, ForeignKey(Filial.codigo), 
                                               nullable=False)
